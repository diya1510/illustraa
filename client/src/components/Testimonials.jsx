...
const Testimonials = () => {
  return (
    <motion.div 
        ...
        <h1 className='text-3xl sm:text-4xl font-semibold mb-2 '>Customers Testimonial</h1>
            ...
            {testimonialsData.map((testimonial, index )=>(
                <div key={index} className='bg-white/20 p-12 rounded-lg shadow-md order w-80 m-auto cursor-pointer hover:scale-[1.02] transition-all'>
                    <div className='flex flex-col items-center'> 
                        ...
                        <p className='text-center text-sm text-gray-600 dark:text-gray-400'>{testimonial.text}</p>
                    </div>
                </div>
            ))}
        </div>
